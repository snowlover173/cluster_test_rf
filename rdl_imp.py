...
from tensorflow.python.ops.initializers_ns import random_uniform
...
class Actor(object):
     def __init__(self, lr, n_actions, name , input_dims, sess, fc1_dims,
                  fc2_dims, action_bound, batch_size=64, chkip_dir='tmp/ddpg'):
        ...
        self.ckpt_dir = chkip_dir
        self.action_bound = action_bound
        self.build_network()
        self.params = tf.compat.v1.trainable_variables(scope=self.name)
        self.saver = tf.compat.v1.train.Saver()
        self.checkpoint_file = os.path.join(self.ckpt_dir, name + '_ddpg.ckpt')
...
class Critic(object):
    def __init__(self, lr , n_actions, name, input_dims, sess,
                 fc1_dims, fc2_dims, batch_size=64, chkpt_dir='tmp/ddpg'):
        self.lr = lr
        self.n_actions = n_actions
        self.name = name
        self.fc1_dims = fc1_dims
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.loss =tf.compat.v1.losses.mean_squared_error(self.q_target,self.q)
                # mean_sqaured_error(self.q_target,self.q)
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
        ...
        self.critic = Critic(beta, n_actions, 'Critic', input_dims,
                             self.sess, layer1_size, layer2_size)

        self.target_actor = Actor(alpha, n_actions,'TargetActor', input_dims, self.sess, layer1_size,
                           layer2_size, env.action_space.high)
